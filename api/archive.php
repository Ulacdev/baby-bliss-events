...
function permanentlyDeleteItem($type, $id)
{
    ...
    $selectStmt = $conn->prepare("SELECT * FROM $archive_table WHERE id = ?");
    ...
    if ($stmt->execute() && $stmt->affected_rows > 0) {
        ...
        $userId = $_SESSION['user_id'] ?? 1;
        $userName = $_SESSION['user_name'] ?? 'Admin';
        $activity = ucfirst($type) . ' Permanently Deleted';
        $details = "Permanently deleted archived " . $type . " for " . ($archived['first_name'] ?? $archived['name'] ?? 'Unknown') . " " . ($archived['last_name'] ?? '') . " (Original ID: " . ($archived['original_id'] ?? $archived['id']) . ")";
        $ipAddress = $_SERVER['REMOTE_ADDR'];
        ...
        sendResponse(['message' => ucfirst($type) . ' permanently deleted']);
    } else {
        sendResponse(['error' => 'Archived item not found'], 404);
    }
}
