...
switch ($method) {
    ...
    case 'DELETE':
        if ($id) {
            ...
            deleteAllBookings();
        } elseif (isset($_GET['all_archived']) && $_GET['all_archived'] === 'true') {
            deleteAllArchivedBookings();
        } else {
            sendResponse(['error' => 'Booking ID required or all=true for bulk delete'], 400);
        }
        break;
    default:
...
function updateBooking($id)
{
    ...
    if ($stmt->execute()) {
        ...
        if (isset($input['status'])) {
            ...
            logAudit('Status Change', "Changed booking ID $id status from {$currentBooking['status']} to {$input['status']} for {$currentBooking['first_name']} {$currentBooking['last_name']}");
        } else {
...
function deleteAllBookings()
{
    ...
    global $user;
    $deleted_by = $user['user_id'] ?? 1;
    $deleted_reason = 'Bulk delete all bookings';
    ...
    $deleteStmt = $conn->prepare("DELETE FROM bookings");
    $deleteStmt->execute();

    logAudit('Bulk Delete', "Deleted all $deletedCount bookings and moved to archive");
    sendResponse(['message' => "All $deletedCount bookings deleted successfully"]);
}

function deleteAllArchivedBookings()
{
    $conn = getDBConnection();
    global $user;
    $deleted_by = $user['user_id'] ?? 1;
    $deleted_reason = 'Bulk permanent delete all archived bookings';

    // Get all archived bookings before deleting
    $stmt = $conn->prepare("SELECT * FROM archived_bookings");
    $stmt->execute();
    $result = $stmt->get_result();

    $deletedCount = 0;
    while ($booking = $result->fetch_assoc()) {
        // Move to permanently deleted table
        $phone = $booking['phone'] ?? '';
        $guests = $booking['guests'] ?? 0;
        $venue = $booking['venue'] ?? '';
        $package = $booking['package'] ?? '';
        ...
        $deleteStmt = $conn->prepare("INSERT INTO permanently_deleted_bookings (original_id, first_name, last_name, email, phone, event_date, guests, venue, package, special_requests, images, status, deleted_reason, deleted_at, original_created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    // Delete all from archived_bookings table
    $deleteStmt = $conn->prepare("DELETE FROM archived_bookings");
    $deleteStmt->execute();

    logAudit('Bulk Permanent Delete', "Permanently deleted all $deletedCount archived bookings");
    sendResponse(['message' => "All $deletedCount archived bookings permanently deleted successfully"]);
}
