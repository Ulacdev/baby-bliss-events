...
function handleDeleteAllUsers()
{
    ...
    if (empty($usersToDelete)) {
        sendResponse(['message' => 'No users to delete'], 200);
        return;
    }
    ...
    try {
        ...
        foreach ($usersToDelete as $userData) {
            ...
            if (!$stmt->execute()) {
                throw new Exception('Failed to delete user: ' . $userData['email']);
            }
            $deletedCount++;
        }

        // Log the action
        $logStmt = $conn->prepare("INSERT INTO audit_logs (user_id, user_name, activity, details, ip_address) VALUES (?, ?, 'Bulk Delete Users', ?, ?)");
        $details = "Deleted $deletedCount users";
        $ipAddress = $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0';
        $logStmt->bind_param("isss", $user['user_id'], $user['email'], $details, $ipAddress);
        $logStmt->execute();
        ...
        sendResponse(['message' => "$deletedCount users deleted successfully"]);
